...
try {
    ...
    return logger.loader("Không thể tải tệp config!", "error");
}

const { Sequelize, sequelize } = require("./source/database");
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async (loginError, loginApiData) => {
        ...
        const listener = require('./source/listen')(listenerData);
...
(async () => {
  try {
      ...
      const sequelize2 = new Sequelize({
          dialect: "sqlite",
          host: __dirname + '/source/antists.sqlite',
          logging: false
      });
      ...
      const models = require('./source/database/model')(authentication);
      const botData = {
          models: models
      };
      onBot(botData);
